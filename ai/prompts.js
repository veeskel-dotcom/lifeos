...
// ═══ Утренний брифинг ═══
export function BRIEFING_PROMPT(data) {
  return `Сгенерируй утренний брифинг на основе данных пользователя.

Данные:
${JSON.stringify(data)}

Правила:
- 3-5 строк, каждая начинается с эмодзи
- Конкретные советы с цифрами
- Не выдумывай — только факты из данных
- Если данных нет по модулю — не упоминай его
- Приоритет: просроченные задачи → платежи → здоровье → бюджет`;
}

// ═══ Еженедельный отчёт ═══
export function WEEKLY_REPORT_PROMPT(weekData, crossData) {
  return `Сгенерируй еженедельный отчёт с инсайтами.

Агрегаты за неделю:
${JSON.stringify(weekData)}

Дневные записи (для корреляций):
${JSON.stringify(crossData)}

Правила:
- Найди 3-5 корреляций между модулями
- Каждый инсайт: наблюдение + рекомендация (одно предложение)
- Конкретные цифры
- Не выдумывай — только то что видишь
